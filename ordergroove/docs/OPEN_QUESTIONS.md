...
3. **Required action fields.** The embedded schemas do not mark `quantity` or `subscription` required even though the operations cannot be meaningful without them. The simulator requires them and labels this as strongly implied.
...
18. **Reset API.** No production reset exists. Simulator state resets only by starting a fresh Counterfact instance; there is no public reset route.
...
- Are `quantity` and `subscription` required request properties in production, and what are their accepted ranges?
