// This code was automatically generated from an OpenAPI description.
// Do not edit this file. Edit the OpenAPI file instead.
// For more information, see https://github.com/pmcelhaney/counterfact/blob/main/docs/faq.md

import type { WideOperationArgument } from "../../../../../../../../counterfact-types/index.ts";
import type { OmitValueWhenNever } from "../../../../../../../../counterfact-types/index.ts";
import type { MaybePromise } from "../../../../../../../../counterfact-types/index.ts";
import type { COUNTERFACT_RESPONSE } from "../../../../../../../../counterfact-types/index.ts";
import type { Context } from "../../../../../../../../routes/_.context.ts";
import type { ResponseBuilderFactory } from "../../../../../../../../counterfact-types/index.ts";
import type { oidc_custom_property_inclusion } from "../../../../../../../components/schemas/oidc-custom-property-inclusion.js";
import type { forbidden } from "../../../../../../../#/components/responses/forbidden.js";
import type { not_found } from "../../../../../../../#/components/responses/not_found.js";
import type { oidc_custom_property_inclusion_input } from "../../../../../../../components/schemas/oidc-custom-property-inclusion-input.js";

/**
 * Lists the repository custom properties that are included in the OIDC token for repository actions in an enterprise.
 *
 * OAuth app tokens and personal access tokens (classic) need the `admin:enterprise` scope to use this endpoint.
 */
export type oidcListOidcCustomPropertyInclusionsForEnterprise = (
  $: OmitValueWhenNever<{
    query: never;
    querystring: never;
    path: oidcListOidcCustomPropertyInclusionsForEnterprise_Path;
    headers: never;
    cookie: never;
    body: never;
    context: Context;
    response: ResponseBuilderFactory<{
      200: {
        headers: never;
        requiredHeaders: never;
        content: {
          "application/json": {
            schema: Array<oidc_custom_property_inclusion>;
          };
        };
        examples: {
          default: unknown;
        };
      };
      403: forbidden;
      404: not_found;
    }>;
    x: WideOperationArgument;
    proxy: (url: string) => COUNTERFACT_RESPONSE;
    user: never;
    delay: (milliseconds: number, maxMilliseconds?: number) => Promise<void>;
    version: never;
  }>,
) => MaybePromise<COUNTERFACT_RESPONSE>;

/**
 * Adds a repository custom property to be included in the OIDC token for repository actions in an enterprise.
 *
 * OAuth app tokens and personal access tokens (classic) need the `admin:enterprise` scope to use this endpoint.
 */
export type oidcCreateOidcCustomPropertyInclusionForEnterprise = (
  $: OmitValueWhenNever<{
    query: never;
    querystring: never;
    path: oidcCreateOidcCustomPropertyInclusionForEnterprise_Path;
    headers: never;
    cookie: never;
    body: oidc_custom_property_inclusion_input;
    context: Context;
    response: ResponseBuilderFactory<{
      201: {
        headers: never;
        requiredHeaders: never;
        content: {
          "application/json": {
            schema: oidc_custom_property_inclusion;
          };
        };
        examples: {
          default: unknown;
        };
      };
      400: {
        headers: never;
        requiredHeaders: never;
        content: never;
        examples: {};
      };
      403: forbidden;
      422: {
        headers: never;
        requiredHeaders: never;
        content: never;
        examples: {};
      };
    }>;
    x: WideOperationArgument;
    proxy: (url: string) => COUNTERFACT_RESPONSE;
    user: never;
    delay: (milliseconds: number, maxMilliseconds?: number) => Promise<void>;
    version: never;
  }>,
) => MaybePromise<COUNTERFACT_RESPONSE>;

export type oidcListOidcCustomPropertyInclusionsForEnterprise_Path = {
  enterprise: string;
};

export type oidcCreateOidcCustomPropertyInclusionForEnterprise_Path = {
  enterprise: string;
};
